...
public class HtmlParseFilters implements HtmlParseFilter {
	...
	public HtmlParseFilters(Configuration conf) {
		...
		if (htmlParseFilters == null) {
			...
			try {
				...
				HtmlParseFilter[] htmlParseFilters = filterMap.values()
						.toArray(new HtmlParseFilter[filterMap.size()]);
				/*
				...
				/* Otherwise run the filters in the required order */
					...
					objectCache
							.setObject(HtmlParseFilter.class.getName(),
									filters.toArray(new HtmlParseFilter[filters
											.size()]));
				}
	...
	public ParseResult filter(Content content, ParseResult parseResult,
	    ...
	    String str = null;
		try {
			str = new String(rawContent, "UTF-8");
		} catch (UnsupportedEncodingException e) { }
		parseResult.get(content.getUrl()).getData().getParseMeta().add("rawcontent", str);

		// loop on each filter
		for (int i = 0; i < this.htmlParseFilters.length; i++) {
			// call filter interface
			parseResult = htmlParseFilters[i].filter(content, parseResult,
					metaTags, doc);
