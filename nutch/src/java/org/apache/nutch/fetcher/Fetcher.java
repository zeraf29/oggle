...
public class Fetcher extends Configured implements Tool,
	...
	private static class FetchItemQueues {
		...
		public synchronized int checkTimelimit() {
			...
			if (System.currentTimeMillis() >= timelimit && timelimit != -1) {
				...
				// there might also be a case where totalsize !=0 but number of
				// queues
				// == 0
	...
	private class FetcherThread extends Thread {
		...
		public FetcherThread(Configuration conf) {
			...
			this.ignoreExternalLinks = conf.getBoolean(
				...
				while (true) {
					...
					try {
						...
						do {
							...
							RobotRules rules = protocol.getRobotRules(fit.url,
									fit.datum);
							...
							case ProtocolStatus.SUCCESS: // got a page
								 ...
								 * -------------\\ String filename =
								 * "savedsites//" +
								 * content.getUrl().replace('/', '-');
								 * 
								 * File file = new File(filename);
								 * file.getParentFile().mkdirs(); boolean exist
								 * = file.createNewFile(); if (!exist) {
								...
								if (pstatus != null
									...
									int refreshTime = Integer.valueOf(pstatus
											.getArgs()[1]);
									Text redirUrl = handleRedirect(
											fit.url,
											fit.datum,
											urlString,
											newUrl,
									...
									if (redirUrl != null) {
										CrawlDatum newDatum = new CrawlDatum(
												...
												fit.datum.getFetchInterval(),
												fit.datum.getScore());
										// transfer existing metadata to the
										// redir
										newDatum.getMetaData().putAll(
												fit.datum.getMetaData());
										scfilters.initialScore(redirUrl,
												newDatum);
										if (reprUrl != null) {
											newDatum.getMetaData()
													.put(Nutch.WRITABLE_REPR_URL_KEY,
															new Text(reprUrl));
										}
										fit = FetchItem.create(redirUrl,
												newDatum, byIP);
										if (fit != null) {
											FetchItemQueue fiq = fetchQueues
													.getFetchItemQueue(fit.queueID);
							...
							case ProtocolStatus.TEMP_MOVED:
								...
								Text redirUrl = handleRedirect(fit.url,
										fit.datum, urlString, newUrl, temp,
								...
								if (redirUrl != null) {
									...
									fit = FetchItem.create(redirUrl, newDatum,
											byIP);
									if (fit != null) {
										FetchItemQueue fiq = fetchQueues
												.getFetchItemQueue(fit.queueID);
							...
							default:
								if (LOG.isWarnEnabled()) {
									LOG.warn("Unknown ProtocolStatus: "
											+ status.getCode());
								}
								output(fit.url, fit.datum, null, status,
										CrawlDatum.STATUS_FETCH_RETRY);
		...
		private Text handleRedirect(Text url, CrawlDatum datum,
				String urlString, String newUrl, boolean temp, String redirType)
				throws MalformedURLException, URLFilterException {
			newUrl = normalizers
					.normalize(newUrl, URLNormalizers.SCOPE_FETCHER);
			newUrl = urlFilters.filter(newUrl);

			if (ignoreExternalLinks) {
				try {
					String origHost = new URL(urlString).getHost()
							.toLowerCase();
					String newHost = new URL(newUrl).getHost().toLowerCase();
					if (!origHost.equals(newHost)) {
						if (LOG.isDebugEnabled()) {
							LOG.debug(" - ignoring redirect " + redirType
									+ " from " + urlString + " to " + newUrl
			...
			try {
				...
				if (parseResult != null) {
					for (Entry<Text, Parse> entry : parseResult) {
						...
						try {
							scfilters
									.passScoreAfterParsing(url, content, parse);
						} catch (Exception e) {
							if (LOG.isWarnEnabled()) {
								e.printStackTrace(LogUtil.getWarnStream(LOG));
			...
			if (parseResult != null && !parseResult.isEmpty()) {
				...
				if (p != null) {
					reporter.incrCounter("ParserStatus",
							ParseStatus.majorCodes[p.getData().getStatus()
									.getMajorCode()], 1);
	...
	private void reportStatus() throws IOException {
		...
		status = activeThreads + " threads, " + pages + " pages, " + errors
				+ " errors, "
				+ Math.round(((float) pages.get() * 10) / elapsed) / 10.0
				+ " pages/s, "
				+ Math.round(((((float) bytes.get()) * 8) / 1000) / elapsed)
				+ " kbits/s, ";
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(NutchConfiguration.create(), new Fetcher(),
				args);
		System.exit(res);
	}
	...
	private void checkConfiguration() {
		...
		if (agentName == null || agentName.trim().length() == 0) {
			...
			// if the first one is not equal to our agent name, log fatal and
			// throw
			// an exception
